...
const collisionCheck = (item1, item2) => {
  ...
  // determine the vector between the two items
  const xVector = item2.x - item1.x;
  const yVector = item2.y - item1.y;
...
export default class StarEater extends Scene{
  ...
  addBall() {
    ...
    this.ball = new Ball(this, randomX, randomY);
  }
  ...
  create() {
    ...
    // add an AI player
    const aiPlayer = new AIPlayer(this, 100, 800, true, this.ball);
  ...
  update() {
    ...
    if (this.ball && this.player && this.aiPlayer) {
      // if player bumps aiPlayer, aiPlayer's velocity is set to react to the collision
      const playerCollision = collisionCheck(this.player, this.aiPlayer);
      if (playerCollision) {
        const { xVector, yVector } = playerCollision;

        console.log('players colided')
        this.aiPlayer.x
        this.aiPlayer.bump(xVector * 0.5, yVector * 0.5);
      }
